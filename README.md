...
Schema validation sees one concept at a time, so `lokf validate` also fails a
bundle in which two files declare one `id` (a sync client's conflict copy is
the usual way), which would otherwise merge into one subject in the graph. It
cannot tell a live relation target from a fabricated one either — both are
...
schema and the `lokf` package carry their own version (currently 0.8.0) so the
