...
Both files are resolved in this order: an explicit ``path`` argument, the
copies packaged under ``lokf/data/`` (regenerated by ``lokf-build``), then a
walk up from the current directory for a repo checkout.
"""
...
def _resolve(name: str, path: str | pathlib.Path | None) -> pathlib.Path:
    ...
    found = _packaged(name) or _from_ancestors(name)
    if found is None:
        raise FileNotFoundError(
            f"{name} not found: not packaged with lokf and not in any "
