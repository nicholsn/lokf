...
import { Content as TrustFields } from "../../../generated/trust-fields-table.md";
...
to a formal vocabulary, so a trust signal is queryable RDF rather than YAML a
consumer has to interpret:

<TrustFields />

All of them are optional, and their absence carries meaning: an unverified
concept is *distinguishable*, never rejected. `generated` supersedes
`timestamp`, and `sources` supersedes `citations`; both older fields are still
parsed. See [SPEC §5.4](/specification/) for the `Source` entry fields and the
...
OKF meaning (v0.2 adds `# Computation`). The body is mapped to `schema:text` in
the RDF projection.
