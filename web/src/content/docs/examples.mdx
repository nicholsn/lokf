...
description: A conformant eight-concept reference bundle for a fictional data org, exercising every headline LOKF feature.
sidebar:
  order: 1
---
...
is a conformant eight-concept reference bundle for a fictional data org. It
exercises every headline feature: typed relationships, type-specific fields,
authorship, and the OKF v0.2 trust, provenance, and attested-computation
families.
...
├── computations/weekly-active-users-count.md  # AttestedComputation: executor + attester
└── references/skills/run-on-bq.md    # Reference: executor instructions
```

## The graph it produces

Eight markdown files, one knowledge graph. Every edge below is a typed
...
graph LR
    ...
    AC["AttestedComputation<br/>weekly-active-users-count"]
    REF["Reference<br/>run-on-bq"]

    WAU -- "prov:wasDerivedFrom" --> UE
    WAU -- "dcterms:requires" --> AU
    WAU -- "lokf:measures" --> AU
    WAU -- "dcterms:requires" --> AC
    AC -- "prov:wasDerivedFrom" --> UE
    AC -- "schema:about" --> AU
    AC -- "dcterms:relation" --> WAU
...
RDF projections: `examples/acme-knowledge.nt` (153 triples for the whole
bundle) and `examples/weekly-active-users.nt` (43 triples for the metric
alone).
