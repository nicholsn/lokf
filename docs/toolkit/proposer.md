...
LOKF keeps [two layers in one file](../guide/relationships.md): human-facing
...
`Relation` from the [vocabulary](../reference/api.md)), a **confidence**
...
[tutorial](tutorial.md#4-propose-typed-relations).
...
- **Reified `relations:` entry** — predicates from the `RelationType`
  ...
  are written as [reified relations](../guide/relationships.md#custom-predicates-relations):
...
!!! warning "It's a heuristic, not an oracle"
