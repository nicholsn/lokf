...
[`GraphStore`](../reference/api.md) (backed by pyoxigraph) and runs SPARQL over
...
`lokf query` is a wrapper around [`GraphStore`](../reference/api.md). The store
...
prepends. To publish the same store over HTTP, see [Serve](serve.md).
